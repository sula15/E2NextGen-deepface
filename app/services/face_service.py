...
class FaceRecognitionService:
    ...
    def compute_face_matches(
        ...
        """
        Compute distances between target embedding and known embeddings.
        Faster than DeepFace.find because it works in memory.

        Args:
            target_embedding: The embedding of the face to identify
        ...
        try:
            ...
            for item in known_embeddings:
                ...
                known_embedding = item.get('embedding')

                if not known_embedding:
                    continue

                known_vector = np.array(known_embedding)

                # Calculate distance based on metric
                if self.distance_metric == 'cosine':
                    # Cosine distance = 1 - cosine_similarity
                    norm_target = np.linalg.norm(target_vector)
                    norm_known = np.linalg.norm(known_vector)

                    if norm_target == 0 or norm_known == 0:
                        distance = 1.0  # Max distance
                    else:
                        dot = np.dot(target_vector, known_vector)
                        similarity = dot / (norm_target * norm_known)
                        # Clamp similarity to avoid numerical errors
                        similarity = max(-1.0, min(1.0, similarity))
                        distance = 1.0 - similarity

                elif self.distance_metric == 'euclidean':
                    distance = np.linalg.norm(target_vector - known_vector)

                elif self.distance_metric == 'euclidean_l2':
                    # L2 Euclidean is just Euclidean on normalized vectors
                    norm_target = np.linalg.norm(target_vector)
                    norm_known = np.linalg.norm(known_vector)

                    if norm_target > 0:
                        norm_t = target_vector / norm_target
                    else:
                        norm_t = target_vector

                    if norm_known > 0:
                        norm_k = known_vector / norm_known
                    else:
                        norm_k = known_vector

                    distance = np.linalg.norm(norm_t - norm_k)
                ...
                if verified:
                    ...
                    identity = item.get('face_image_path', str(user_id))
                    results.append({
                        'user_id': user_id,
                        'identity': identity,
            ...
            # Don't raise, just return empty list as this is a helper method
            return []
    ...
    def recognize_face(
        ...
        Args:
            ...
            known_embeddings: Optional list of known embeddings (optimization)
        ...
        try:
            ...
            if known_embeddings and len(known_embeddings) > 0:
                count = len(known_embeddings)
                logger.info(f"Using in-memory embeddings: {count} users")

                # Extract embedding from target image
                target_embedding = self.extract_embedding(img_path)

                if target_embedding:
                    matches = self.compute_face_matches(
                        target_embedding,
                        known_embeddings
                    )
                    count = len(matches)
                    logger.info(f"Found {count} matches (in-memory)")
                    return matches
                else:
                    logger.warning("Could not extract embedding from target")
                    return []

            # FALLBACK: Use DeepFace.find
