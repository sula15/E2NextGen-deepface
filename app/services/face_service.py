...
class FaceRecognitionService:
    ...
    def __init__(self, model_name='Facenet',
        ...
        logger.info(f"FaceRecognitionService initialized with "
    ...
    def compute_face_matches(
        ...
        """
        Compute matches using in-memory embeddings (Vectorized)

        Args:
            target_embedding: The embedding of the face to recognize
        ...
        try:
            ...
            if self.distance_metric == 'cosine':
                ...
                dot_product = np.dot(known_matrix, target_vector)
                cosine_similarity = dot_product / (known_norms * target_norm)
                distances = 1 - cosine_similarity

            elif self.distance_metric == 'euclidean':
                # Euclidean distance = ||A - B||
                distances = np.linalg.norm(
                    known_matrix - target_vector, axis=1
                )

            elif self.distance_metric == 'euclidean_l2':
                # L2 Euclidean distance = ||A - B|| where A, B are normalized
                target_norm = np.linalg.norm(target_vector)
                known_norms = np.linalg.norm(known_matrix, axis=1)

                target_vector_norm = target_vector / (
                    target_norm if target_norm > 0 else 1e-10
                )
                known_matrix_norm = known_matrix / (
                    known_norms[:, np.newaxis] + 1e-10
                )

                distances = np.linalg.norm(
                    known_matrix_norm - target_vector_norm, axis=1
                )
            ...
            if threshold is not None:
                ...
                effective_threshold = self._get_default_threshold()
            ...
            for i, distance in enumerate(distances):
                if distance < effective_threshold:
                    results.append({
                        'user_id': user_ids[i],
                        # Placeholder
                        'identity': f"DB_USER_{user_ids[i]}",
                        ...
                        try:
                            ...
                            if distance_col in row:
                                ...
                                if distance_cols:
                                    ...
                                    logger.warning(
                                        f"Could not find distance column. "
            ...
            for user_dir in db_path_obj.iterdir():
                ...
                for img_file in user_dir.glob('*.jpg'):
                    try:
                        ...
                        logger.error(f"Error comparing with {img_file}: "
                                     f"{str(e)}")
                        continue
    ...
    def _get_default_threshold(self) -> float:
        ...
        return thresholds.get(self.model_name, {}).get(
            self.distance_metric, 0.40)
