...
def recognize():
    ...
    try:
        ...
        try:
            ...
            # 1. Extract embedding from upload
            target_embedding = face_service.extract_embedding(img_path)
            ...
            # 2. Fetch active user embeddings from DB
            # Use .with_entities to only fetch what we need for matching
            users_data = User.query.filter(
                User.is_active == True,
                User.embedding.isnot(None)
            ).with_entities(User.user_id, User.embedding).all()

            known_embeddings = [
                {'user_id': u.user_id, 'embedding': u.embedding}
                for u in users_data
            ]
            ...
            if not known_embeddings:
                ...
                results = face_service.compute_face_matches(target_embedding, known_embeddings)
            ...
            if results and len(results) > 0:
                ...
                if user:
                    ...
                    if data.get('log_attendance', False):
                        if best_match['threshold'] > 0:
                            conf = 1 - (
                                best_match['distance'] / best_match['threshold']
                            )
                        else:
                            conf = 0
...
def enroll():
    ...
    try:
        ...
        img_data = base64.b64decode(
            data['image'].split(',')[1]
            if ',' in data['image'] else data['image']
        )
